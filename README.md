...
`ai-platform run` starts the existing polling worker flow. `ai-platform codex-exec` starts a single non-interactive `codex exec` run. `ai-platform watch` starts the file watcher that monitors `ai/tasks/pending` and triggers the non-interactive execution flow when tasks appear. Completion is reported through `ai/status/latest-run.json` and `ai/status/latest-codex-summary.md`.
...
`ai-platform implement` v1 selects a pending task, validates basic metadata, can move it to `ai/tasks/in-progress`, and writes `ai/reports/implementation-prompt.md` for Codex. It also prints the recommended next command, `ai-platform task move --task TASK-xxxx --to review`, for use after implementation and validation. It does not execute Codex, implement code automatically, move tasks to `review` or `done`, commit, or push. The Codex execution step still must implement the task, validate it, commit, push, and then move the task to review explicitly when appropriate. Final closure still requires review and an explicit `task move --to done` when supported by evidence.
