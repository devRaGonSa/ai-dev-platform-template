...
| R-004 | Read-only analysis commands | in-progress | Inspect repository state without modifying files. | Users can understand platform health, roadmap progress, and gaps safely. | `analyze`, `roadmap-status` |
...
| R-006 | Reconciliation commands | planned | Compare roadmap, code, docs, and tasks to detect drift. | Mismatches between intended direction and actual implementation are visible. | `reconcile` |
...
| R-008 | Review workflow | planned | Formalize review states before tasks are completed. | Implementation and review are distinct before tasks are marked done. | `review` |
